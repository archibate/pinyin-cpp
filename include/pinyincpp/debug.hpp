...
private:
    ...
    static void uni_format(std::ostream &oss, T0 &&t) {
        ...
        } else if constexpr (requires (T const &t) { t.tuplify(); }) {
            uni_format(oss, t.tuplify());
        /* } else if constexpr (requires (T const &t) { uni_tuplify(t); }) { */
        /*     uni_format(oss, uni_tuplify(t)); */
        } else {
            oss << '<' << typeid(t).name() << '@' << reinterpret_cast<void const *>(std::addressof(t)) << '>';
        }
    }

#if !defined(NDEBUG)
    static thread_local inline std::unordered_map<std::string, std::string> fileCache;
#endif
